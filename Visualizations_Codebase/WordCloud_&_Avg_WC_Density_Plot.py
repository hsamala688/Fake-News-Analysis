...
fake_text = preprocess_text_case_sensitive_v2(
    " ".join(fake_news_df["text"].astype(str).tolist()), set(stopwords.words("english"))
)

real_text = preprocess_text_case_sensitive_v2(
    " ".join(real_news_df["text"].astype(str).tolist()), set(stopwords.words("english"))
)
...
fake_wordcloud = WordCloud(
    width=1200, height=600, background_color="white", regexp=r"\w[\w']*\w|\w"
).generate(fake_text)
...
fake_news_df["word_count"] = (
    fake_news_df["text"].astype(str).apply(lambda x: len(x.split()))
)
real_news_df["word_count"] = (
    real_news_df["text"].astype(str).apply(lambda x: len(x.split()))
)
...
sns.kdeplot(
    fake_news_df["word_count"], label="Fake News", color="red", fill=True, alpha=0.5
)
sns.kdeplot(
    real_news_df["word_count"], label="Real News", color="blue", fill=True, alpha=0.5
)
